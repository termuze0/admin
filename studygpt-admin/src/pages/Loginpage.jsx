...
function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Clear previous tokens to avoid conflicts
      localStorage.removeItem('tokens');
      localStorage.removeItem('user');

      const response = await fetch('http://56.228.80.139/api/account/login/', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="login-container">
      ...
      <form onSubmit={handleSubmit} className="login-form">
        ...
        >
          {isLoading ? (
            <span className="loading-spinner" aria-label="Loading">Loading...</span>
          ) : (
            'Sign In'
          )}
        </button>
